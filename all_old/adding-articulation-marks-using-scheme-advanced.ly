%% http://lsr.di.unimi.it/LSR/Item?id=696
...
%% see also LSR = http://lsr.dsi.unimi.it/LSR/Item?u=1&id=769
%% last changes : 
       ...
       (case (name-of evt)
         ((EventChord)
           (receive (notes others)
             ...
             (if (pair? notes)(set! (cons others res)))))
             ; keeps only not note events
         ((NoteEvent)(set! res
             (cons (ly:music-property evt 'articulations) res)))
         (else #f)))
     pattern)
   (reverse res)))
